...
func ProfileConfigHash(cfg config.RecommendationConfig, embeddingVersion string) string {
	canonical := fmt.Sprintf(
		"profile=%s|canonical=%s|embedding=%s|recent_views=%d|replies=%d|view=%g|like=%g|click=%g|qualified_read=%g|reply=%g|quick_bounce=%g|not_interested=%g|half_life=%g|lookback=%d|coexist=%g|post_cap=%g",
		...
		cfg.PositiveSignalCoexistBonus, cfg.PositivePostWeightCap,
	)
