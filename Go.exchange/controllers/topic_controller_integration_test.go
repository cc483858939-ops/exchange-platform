...
func TestTopicFeedMembershipOrderingAndCursorIntegration(t *testing.T) {
	...
	newPost := func(author models.User, label string, createdAt time.Time, visibility string) models.Post {
		t.Helper()
		post := models.Post{Model: gorm.Model{CreatedAt: createdAt, UpdatedAt: createdAt}, AuthorID: author.ID, Content: label, Visibility: visibility}
	...
	older := newPost(author, "older", baseTime.Add(-time.Hour), "public")
	tieLow := newPost(author, "tie-low", baseTime, "public")
	tieHigh := newPost(author, "tie-high", baseTime, "public")
	private := newPost(author, "private", baseTime.Add(time.Hour), "private")
	tombstoned := newPost(author, "tombstoned", baseTime.Add(2*time.Hour), "public")
	deleted := newPost(author, "deleted", baseTime.Add(3*time.Hour), "public")
	disabled := newPost(author, "disabled-account", baseTime.Add(4*time.Hour), "public")
	unrelated := newPost(author, "unrelated-account", baseTime.Add(5*time.Hour), "public")
	...
	mapPost(includedAccount, private, models.DevDataMirrorPostStateActive)
	...
	postIDs := []uint{older.ID, tieLow.ID, tieHigh.ID, private.ID, tombstoned.ID, deleted.ID, disabled.ID, unrelated.ID}
	userIDs := []uint{includedAccount.LocalUserID, disabledAccount.LocalUserID, otherAccount.LocalUserID, author.ID}
	t.Cleanup(func() {
