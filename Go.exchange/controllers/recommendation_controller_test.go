...
func TestNormalizedRulesV3RecommendationConfigDefaultsWhenMissing(t *testing.T) {
...
func TestNormalizedRulesV3RecommendationConfigFallsBackForNonPositiveValues(t *testing.T) {
...
func TestRulesV3ProfileUsesReactionAndIgnoresViewCount(t *testing.T) {
	now := time.Date(2026, 6, 9, 12, 0, 0, 0, time.UTC)
	backend := recommendationTestArticle(1, now, "Backend", []string{"Go"}, 0)
	...
	if profile.PersonalizedSignalCount != 2 {
		t.Fatalf("expected one view and one like signal, profile=%#v", profile)
	}
...
func TestRulesV3ScoreUsesConfiguredWeights(t *testing.T) {
	withRecommendationConfig(t, config.RecommendationConfig{
		CategoryWeight:   5,
		TagWeight:        7,
		PopularityWeight: 11,
	...
	got := scoreRulesV3Article(profile, article, now, normalizedRulesV3RecommendationConfig())
	want := 2*5 + 3*7 + math.Log(2)*11 + 13
	if math.Abs(got-want) > 0.000001 {
		t.Fatalf("unexpected configured score: got %f want %f", got, want)
	}
}

func TestRulesV3RecommendArticlesPrefersMatchingSignals(t *testing.T) {
	now := time.Date(2026, 6, 9, 12, 0, 0, 0, time.UTC)
	article := recommendationTestArticle(1, now, "Backend", []string{"Go", "AI"}, 0)
	profile := buildRulesV3InterestProfile([]articleBehaviorSignal{{Behavior: models.ArticleBehavior{ArticleID: 1, Action: ArticleBehaviorActionView, LastSeenAt: now}, Article: article}}, nil, map[uint]recommendationReactionState{
		1: {Liked: true, StateChangedAt: now},
	}, now, normalizedRulesV3RecommendationConfig())
	recommendations := recommendRulesV3Articles(profile, []models.Article{
	...
	}, now, normalizedRulesV3RecommendationConfig(), 10)

	if len(recommendations) != 2 || recommendations[0].ID != 3 {
		t.Fatalf("expected matching article first, recommendations=%#v", recommendations)
	}
	_ = article
}

func TestRulesV3RecommendArticlesFallsBackToPopularityAndFreshnessWithoutProfile(t *testing.T) {
	now := time.Date(2026, 6, 9, 12, 0, 0, 0, time.UTC)
	cfg := normalizedRulesV3RecommendationConfig()
	profile := buildRulesV3InterestProfile(nil, nil, nil, now, cfg)
	recommendations := recommendRulesV3Articles(profile, []models.Article{
	...
	if len(recommendations) != 2 || recommendations[0].ID != 1 {
		t.Fatalf("expected popular article first, recommendations=%#v", recommendations)
	}
}

func TestRulesV3RecommendArticlesExcludesInteractedAndClampsLimit(t *testing.T) {
	now := time.Date(2026, 6, 9, 12, 0, 0, 0, time.UTC)
	profile := userInterestProfile{
	...
	recommendations := recommendRulesV3Articles(profile, candidates, now, normalizedRulesV3RecommendationConfig(), 99)
...
func TestGetArticleRecommendationsReturnsSortedPayload(t *testing.T) {
	...
	originalBehavior := loadRecommendationBehaviorSignals
	originalFeedback := loadRecommendationFeedbackSignals
	originalReactions := loadRecommendationReactionStates
	originalCandidates := loadRulesV3Candidates
	t.Cleanup(func() {
		loadRecommendationBehaviorSignals = originalBehavior
		loadRecommendationFeedbackSignals = originalFeedback
		loadRecommendationReactionStates = originalReactions
		loadRulesV3Candidates = originalCandidates
	})
	...
	loadRecommendationBehaviorSignals = func(userID uint) ([]articleBehaviorSignal, error) {
		...
		return []articleBehaviorSignal{{
			Behavior: models.ArticleBehavior{ArticleID: 1, Action: ArticleBehaviorActionView, Count: 20, LastSeenAt: now},
			Article:  recommendationTestArticle(1, now, "Backend", []string{"Go"}, 0),
		}}, nil
	}
	loadRecommendationFeedbackSignals = func(uint, time.Time) ([]recommendationFeedbackSignal, error) {
		return nil, nil
	}
...
func TestGetArticleRecommendationsSerializesNilTagsAsArray(t *testing.T) {
	...
	if len(payload) != 1 || string(payload[0]["tags"]) != "[]" {
		t.Fatalf("expected nil tags to serialize as [], payload=%s", recorder.Body.String())
	}
}
