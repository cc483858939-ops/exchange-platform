...
func WorkerReady() bool {
	notificationsReady := !notificationConsumerConfigured() || notificationConsumers.Load() > 0
	return workerReady.Load() && notificationsReady && articleAnalysisConsumers.Load() > 0 && userBehaviorConsumers.Load() > 0 && recommendationMetricsConsumers.Load() > 0 && likeSnapshotConsumers.Load() > 0 && likeBehaviorRelayWorkers.Load() > 0 && likeSnapshotRelayRunning.Load()
}
