...
Feed dwell deliberately does not instrument Following items, recently published local posts, or RecommendationView's masonry cards. Existing recommendation impressions still require raw IntersectionObserver visibility of at least 50% for at least one second. Feed dwell does not require a corresponding impression. This can happen for two independent reasons: (1) Feed dwell may finalize before the existing one-second impression delay; or (2) Feed dwell uses normalized viewport visibility while impression retains raw IntersectionObserver intersectionRatio semantics, so a card taller than the viewport may accumulate Feed dwell even when its raw ratio never reaches 50%. `feed_dwell_count` may therefore exceed impression relationships; this difference is intentional.
