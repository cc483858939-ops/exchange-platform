...
vi.mock('./sessionSync', () => ({
    registerHistorySessionSync: vi.fn((sync) => { mocks.historySync = sync; }),
    syncExternalPostLikeState: vi.fn((update) => {
        mocks.historySync?.applyExternalLikeStateLocal(update);
    }),
    syncExternalPostRepostState: vi.fn((update) => {
        mocks.historySync?.applyExternalRepostStateLocal(update);
    }),
}));
const post = (id, authorID = 9) => ({
    id,
...
const loadReady = async (store, ids) => {
