...
import { registerHistorySessionSync, syncExternalPostLikeState, syncExternalPostRepostState, } from './sessionSync';
const pageSize = 20;
const normalizeID = (value) => (typeof value === 'number' && Number.isSafeInteger(value) && value > 0 ? value : null);
...
export const useHistorySessionStore = defineStore('historySession', () => {
    ...
    const pendingUnlikePostIDs = ref(new Set());
    const repostPendingPostIDs = ref(new Set());
    const mutationErrors = ref(new Map());
    const loadedPostIDs = new Set();
    const removedSnapshots = new Map();
    const deletedPostIDs = new Set();
    const likeMutationVersions = reactive(new Map());
    const repostMutationVersions = reactive(new Map());
    ...
    const isCurrentViewer = (capturedViewerID, capturedGeneration) => (authStore.isAuthenticated
        && viewerID.value === capturedViewerID
        && viewerGeneration.value === capturedGeneration);
    const isCurrentRequest = (capturedRequestVersion, capturedViewerID, capturedGeneration) => (requestVersion.value === capturedRequestVersion
        && isCurrentViewer(capturedViewerID, capturedGeneration));
    const findPost = (postID) => items.value.find(post => post.id === postID);
    const updateSnapshotLikeState = (postID, update) => {
    ...
    const updateSnapshotRepostState = (postID, update) => {
        const snapshot = removedSnapshots.get(postID);
        if (!snapshot) {
            return false;
        }
    ...
    const removePostWithSnapshot = (postID) => {
    ...
    const restoreSnapshot = (postID, update) => {
    ...
    const appendHistoryPosts = (posts) => {
        const additions = [];
        posts.forEach((post) => {
    ...
    const hydrateLikeStates = async (posts, capturedRequestVersion, capturedViewerID, capturedGeneration) => {
        const postIDs = Array.from(new Set(posts.map(post => post.id)));
        if (postIDs.length === 0) {
            return;
        }
        const hydrationGeneration = likeHydrationGeneration.value;
        const capturedMutationVersions = new Map(postIDs.map(postID => [postID, getLikeMutationVersion(postID)]));
    ...
    const markRepostUnavailableLocal = (posts, versions) => {
        posts.forEach((post) => {
    ...
    const hydrateRepostStates = async (posts, capturedRequestVersion, capturedViewerID, capturedGeneration) => {
        const postIDs = Array.from(new Set(posts.map(post => post.id)));
        if (postIDs.length === 0)
            return;
        const hydrationGeneration = repostHydrationGeneration.value;
        const capturedRepostGeneration = repostGeneration;
        const capturedMutationVersions = new Map(postIDs.map(postID => [postID, getRepostMutationVersion(postID)]));
        const current = () => (isCurrentRequest(capturedRequestVersion, capturedViewerID, capturedGeneration)
        ...
        try {
            ...
            if (current())
                markRepostUnavailableLocal(posts, capturedMutationVersions);
        }
    };
    ...
    const revalidateHistory = async () => {
        ...
        try {
            ...
            const cachedTail = oldItems.filter(post => (!deletedPostIDs.has(post.id) && !freshIDs.has(post.id)));
            ...
            void hydrateLikeStates(freshForHydration, capturedRequestVersion, capturedViewerID, capturedGeneration);
    ...
    const toggleUnlike = async (postID) => {
        ...
        const isCurrentMutation = () => (isCurrentViewer(capturedViewerID, capturedGeneration)
        ...
        try {
            ...
            if (snapshot) {
                const restored = {
                    ...snapshot.post,
                    likeStatus: getErrorStatus(error) === 503 ? 'unavailable' : 'ready',
                };
                const nextItems = [...items.value];
            ...
            mutationErrors.value.set(postID, getErrorStatus(error) === 503
    ...
    const toggleRepost = async (postID) => {
        ...
        const isCurrentMutation = () => (isCurrentViewer(capturedViewerID, capturedViewerGeneration)
    ...
    const applyExternalRepostStateLocal = (update) => {
        ...
        if (snapshot)
            return applyFeedRepostStateUpdate(snapshot.post, update);
        return false;
    };
