...
const sameQuoteForm = (left, right) => (left.fromCurrency === right.fromCurrency
...
export const useExchangeSessionStore = defineStore('exchangeSession', () => {
    ...
    const updateFormCurrencies = (available) => {
    ...
    const loadCurrencies = async (options = {}) => {
        ...
        try {
            const { data } = await axios.get('/exchange/currencies');
    ...
    const requestQuote = async () => {
        ...
        quoteError.value = '';
        try {
            const { data } = await axios.get('/exchange/quote', {
                params: {
