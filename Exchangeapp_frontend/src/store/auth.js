...
const asAuthorizationHeader = (rawToken) => {
...
export const useAuthStore = defineStore('auth', () => {
    ...
    const token = ref(asAuthorizationHeader(storedAccessToken));
    const refreshToken = ref(localStorage.getItem(refreshTokenKey));
    const identity = ref(loadStoredIdentity() || decodeAuthIdentity(storedAccessToken));
    const isAuthenticated = computed(() => Boolean(token.value && refreshToken.value && identity.value));
    const currentIdentity = computed(() => identity.value);
    ...
    const syncCurrentIdentityProfile = (candidate) => {
