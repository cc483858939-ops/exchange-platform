...
vi.mock('./sessionSync', () => ({
    registerConnectionsSessionSync: vi.fn((sync) => { mocks.connectionsSync = sync; }),
    syncExternalFollowState: vi.fn((state) => {
...
const followResponse = (id, following) => ({
    user_id: id,
    following,
...
describe('connectionsSession store', () => {
    beforeEach(() => {
        ...
        mocks.getUser.mockImplementation((id) => Promise.resolve(user(Number(id))));
    ...
    it('keeps only eight target sessions and invalidates them on viewer change', async () => {
        const store = createStore();
        mocks.getSession.mockImplementation((id) => ({ user: user(Number(id)) }));
        for (let id = 1; id <= 9; id += 1)
            store.activate(id, 'followers');
