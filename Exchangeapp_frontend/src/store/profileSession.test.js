...
const followState = (id, following) => ({
    user_id: id,
    following,
...
describe('profile session store', () => {
    beforeEach(() => {
        ...
        mocks.getUser.mockReset().mockImplementation((id) => Promise.resolve(profile(Number(id))));
    ...
    it('keeps an unrelated pending initial Post request valid when another Post is removed', async () => {
        ...
        mocks.feedStore.isPostDeleted.mockImplementation((postID) => postID === 42);
        const store = useProfileSessionStore();
    ...
    it('external follow state invalidates an older Profile follow request', async () => {
        let resolveFollow;
        const pendingFollow = new Promise((resolve) => {
