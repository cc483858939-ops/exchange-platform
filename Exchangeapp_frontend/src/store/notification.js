...
import { getNotifications, getUnreadNotificationCount, markAllNotificationsRead, markNotificationRead as markNotificationReadRequest, } from '../services/notificationService';
...
const mergeUniqueNotifications = (existing, incoming) => {
    const seen = new Set();
    const merged = [];
...
export const useNotificationStore = defineStore('notification', () => {
    ...
    const revalidateError = ref(null);
    ...
    const isCurrentViewer = (capture) => (capture !== null
    ...
    const refreshUnreadCount = async (capture = captureViewer()) => {
    ...
    const isCurrentListRequest = (capture, requestVersion, pagingVersion) => (isCurrentViewer(capture)
    ...
    const revalidateNotifications = async () => {
        ...
        try {
            ...
            const readOverrides = new Map(items.value
                .filter(item => markAllPending.value || pendingReadIDs.value.has(item.id))
                .map(item => [item.id, item.read]));
            items.value = [...fresh, ...cachedTail].map(item => (readOverrides.has(item.id) ? { ...item, read: readOverrides.get(item.id) ?? item.read } : item));
            nextCursor.value = response.next_cursor;
            listStale.value = false;
            revalidateError.value = null;
        }
