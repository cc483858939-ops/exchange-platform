...
const recommendationPage = (items, depleted = false) => ({
    items,
    request_id: 'request-id',
    depleted,
});
const followingActivity = (id, postAuthorID = 7, actorID = postAuthorID, activityType = 'post') => ({
...
describe('home timeline session store', () => {
    ...
    it('deduplicates page responses and skips locally deleted posts', async () => {
        mocks.feedStore.isPostDeleted.mockImplementation((id) => id === 2);
        mocks.getPostRecommendations
            .mockResolvedValueOnce(recommendationPage([recommendation(1), recommendation(2), recommendation(3)]))
    ...
    it('external like state invalidates an older local like mutation', async () => {
        let resolveLike;
        const pendingLike = new Promise((resolve) => {
            resolveLike = resolve;
        });
...
const feedPostFixture = (id, authorID) => ({
    id,
    author: author(authorID),
    content: `Post ${id}`,
    media: [],
