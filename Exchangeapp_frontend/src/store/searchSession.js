...
const normalizeViewerID = (value) => (typeof value === 'number' && Number.isSafeInteger(value) && value > 0 ? value : null);
export const useSearchSessionStore = defineStore('searchSession', () => {
    ...
    const currentPageRequest = (capturedPageGeneration, capturedRequestVersion, capturedQuery, capturedViewerID, capturedViewerGeneration) => (capturedPageGeneration === pageGeneration.value
    ...
    const loadInitial = async (force = false) => {
        ...
        try {
            ...
            if (!currentPageRequest(capturedPageGeneration, capturedRequestVersion, capturedQuery, capturedViewerID, capturedViewerGeneration)) {
                return;
            }
            appendPage(page);
        }
    ...
    const toggleFollow = async (userID) => {
        ...
        const mutationCurrent = () => (capturedViewerID === viewerID.value
            && capturedViewerGeneration === viewerGeneration.value
