...
import { deletePost as deletePostRequest, } from '../services/postService';
import { followUser, getUser, getUserPosts, getUserFollowState, unfollowUser, } from '../services/userService';
...
import { registerProfileSessionSync, syncProfilePostRemoval, syncProfileAuthorIdentity, syncProfileLikeState, syncProfileRepostState, } from './sessionSync';
...
export const useProfileSessionStore = defineStore('profileSession', () => {
    ...
    const deleteTargetProfileIDs = new Map();
    const deleteMutationVersions = new Map();
    ...
    const getSession = (rawUserID) => {
        const userID = normalizeID(rawUserID);
        if (userID === null)
            return null;
        return sessions.get(userID) || null;
    };
    const isCurrentSessionCapture = (capture) => {
    ...
    const captureSession = (rawUserID) => {
        const userID = normalizeID(rawUserID);
    ...
    const forEachProfilePost = (postId, callback) => {
    ...
    const findPost = (postId, rawUserID) => {
        ...
        if (preferredPost && !feedStore.isPostDeleted(postId))
    ...
    const applyLikeStateUpdateLocal = (update) => {
        let applied = false;
    ...
    const applyExternalLikeStateLocal = (update) => {
        likeMutationVersions.set(update.postId, (likeMutationVersions.get(update.postId) ?? 0) + 1);
    ...
    const applyReplyCountUpdateEverywhereLocal = (update) => {
    ...
    const hydrateLikeStates = async (postIds, capturedViewerGeneration, isCurrent) => {
    ...
    const appendPosts = (session, rawPosts) => {
        const newPosts = rawPosts
            .filter((post) => {
            if (session.loadedPostIds.has(post.id))
                return false;
    ...
    const currentRequest = (userID, session, version, capturedViewerID, capturedViewerGeneration) => sessions.get(userID) === session
        && session.postRequestVersion === version
        && viewerID.value === capturedViewerID
        && viewerGeneration.value === capturedViewerGeneration;
    const currentPostSession = (userID, session, capturedPostsGeneration, capturedViewerID, capturedViewerGeneration) => sessions.get(userID) === session
        && session.postsGeneration === capturedPostsGeneration
        && viewerID.value === capturedViewerID
        && viewerGeneration.value === capturedViewerGeneration;
    const loadPosts = async (rawUserID, force = false) => {
        const userID = normalizeID(rawUserID);
        const session = userID === null ? null : ensureSession(userID);
        if (!userID || !session || (session.postsInitialLoading && !force))
            return session;
        if (session.postsLoaded && !force)
            return session;
        if (force) {
        ...
        try {
            ...
            void hydrateLikeStates(newPosts.map((post) => post.id), capturedViewerGeneration, () => currentPostSession(userID, session, capturedPostsGeneration, capturedViewerID, capturedViewerGeneration));
            void hydrateRepostStates(newPosts.map((post) => post.id), () => currentPostSession(userID, session, capturedPostsGeneration, capturedViewerID, capturedViewerGeneration));
        }
        catch (error) {
            if (currentRequest(userID, session, requestVersion, capturedViewerID, capturedViewerGeneration)) {
    ...
    const loadMorePosts = async (rawUserID) => {
        const userID = normalizeID(rawUserID);
        const session = userID === null ? null : ensureSession(userID);
        if (!userID
            || !session
    ...
    const retryLoadMorePosts = (rawUserID) => {
    ...
    const loadFollowState = async (rawUserID, force = false) => {
        const userID = normalizeID(rawUserID);
        const session = userID === null ? null : ensureSession(userID);
        const capturedViewerID = viewerID.value;
        ...
        if (session.followLoaded && !force)
            return session;
        if (session.followLoading && !force)
            return session;
        if (force)
            session.followRequestVersion += 1;
        ...
        try {
            ...
            if (!isCurrent())
                return session;
            if (response.user_id !== undefined && response.user_id !== userID)
    ...
    const toggleFollow = async (rawUserID) => {
        const userID = normalizeID(rawUserID);
    ...
    const toggleLike = async (postId, rawUserID) => {
        ...
        const isCurrent = () => likePendingPostIds.has(postId)
    ...
    const toggleRepost = async (postId, rawUserID) => {
        const post = findPost(postId, rawUserID);
        const capturedViewerID = viewerID.value;
        if (!post
    ...
    const removePostEverywhereLocal = (postId) => {
        sessions.forEach((session) => {
    ...
    const removePostEverywhere = (postId, ownerUserID) => {
        if (ownerUserID !== undefined && !feedStore.markPostDeleted(postId, ownerUserID))
            return false;
        removePostEverywhereLocal(postId);
        if (ownerUserID !== undefined)
            syncProfilePostRemoval(postId);
        return true;
    };
    const deletePost = async (postId, rawUserID) => {
        ...
        if (targetUserID !== null)
            deleteTargetProfileIDs.set(postId, targetUserID);
    ...
    const cancelPendingDeletesForProfile = (rawUserID) => {
        const userID = normalizeID(rawUserID);
        if (userID === null)
            return;
    ...
    const replaceAuthorIdentityEverywhereLocal = (author) => {
        sessions.forEach((session) => {
    ...
    const replaceAuthorIdentityEverywhere = (author) => {
    ...
    const loadProfile = async (rawUserID, force = false) => {
        const userID = normalizeID(rawUserID);
        const session = userID === null ? null : ensureSession(userID);
        if (!userID || !session)
            return session;
        if (session.profileLoading && !force)
            return session;
        if (session.profileLoaded && !force) {
            if (!session.postsLoaded && !session.postsInitialLoading)
        ...
        try {
            ...
            void loadPosts(userID);
            if (viewerID.value !== null && authStore.isAuthenticated)
                void loadFollowState(userID);
        }
        catch (error) {
            if (!isCurrent())
    ...
    const setScrollY = (rawUserID, value) => {
        const session = ensureSession(rawUserID);
        if (session)
            session.scrollY = Number.isFinite(value) && value >= 0 ? value : 0;
    };
