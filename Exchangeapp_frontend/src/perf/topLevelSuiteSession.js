import { PERF_EXECUTIONS_PER_SCENARIO, PERF_MAX_RECORDED_RETRIES, PERF_RECORDED_RUNS, scenarioPlanLabel, } from './runnerPlan';
import { viewportDefaults } from './scenarioConfig';
export const PERF_SESSION_SCHEMA_VERSION = 2;
...
export const isViewportWithinTolerance = (actualWidth, actualHeight, targetWidth, targetHeight, tolerance = PERF_VIEWPORT_TOLERANCE) => (Number.isFinite(actualWidth)
    && Number.isFinite(actualHeight)
    && Math.abs(actualWidth - targetWidth) <= tolerance
    && Math.abs(actualHeight - targetHeight) <= tolerance);
export const perfViewportRequirement = (viewport) => ({
    viewport,
    width: viewportDefaults[viewport].width,
    height: viewportDefaults[viewport].height,
    tolerance: PERF_VIEWPORT_TOLERANCE,
});
export const isScenarioViewportValid = (scenario) => {
    const target = viewportDefaults[scenario.viewport];
    return isViewportWithinTolerance(scenario.innerWidth, scenario.innerHeight, target.width, target.height);
};
export const isExecutionEnvironmentConsistent = (run, environment) => (run.executionContext.userAgent === environment.userAgent
    ...
    && Math.abs(run.executionContext.devicePixelRatio - environment.devicePixelRatio)
        <= PERF_DPR_TOLERANCE);
export const executionContextValidationIssues = (run, environment) => {
    const issues = [];
    ...
    if (!isScenarioViewportValid(run.scenario)) {
        ...
        issues.push(`viewport ${run.scenario.innerWidth}x${run.scenario.innerHeight} is outside `
...
function buildInitialSession(options) {
    return {
...
export function createSuiteSession(optionsOrSuiteId, harnessHead, environment, createdAt) {
    const options = typeof optionsOrSuiteId === 'string'
        ? {
...
export const serializeSuiteSession = (session) => (JSON.stringify(session));
const isRecord = (value) => (Boolean(value) && typeof value === 'object');
const isNonEmptyString = (value) => (typeof value === 'string' && value.trim().length > 0);
const isNonNegativeInteger = (value) => (Number.isSafeInteger(value) && value >= 0);
const isFiniteNumber = (value) => (typeof value === 'number' && Number.isFinite(value));
const isStringArray = (value) => (Array.isArray(value) && value.every(item => typeof item === 'string'));
const hasFiniteFields = (value, fields) => fields.every(field => isFiniteNumber(value[field]));
export const isPerfRawRun = (value) => {
    if (!isRecord(value)
...
const isValidCursor = (value) => {
    ...
    if (value.phase !== 'warmup' && value.phase !== 'recorded')
        return false;
    return value.phase === 'warmup'
        ? value.recordedIndex === 0 && value.attemptNumber === 1
        : true;
};
export const isPerfTopLevelSuiteSession = (value) => {
    if (!isRecord(value)
        ...
        || !value.failures.every(value => (isRecord(value)
...
export const parseSuiteSession = (serialized) => {
...
export const serializePendingScenarioEnvelope = (envelope) => (JSON.stringify(envelope));
export const isPerfPendingScenarioEnvelope = (value) => {
    if (!isRecord(value)
        || value.schemaVersion !== PERF_SESSION_SCHEMA_VERSION
        || !isNonEmptyString(value.suiteId)
    ...
    if (value.type === 'result')
        return isPerfRawRun(value.result);
    return isNonEmptyString(value.error);
};
export const parsePendingScenarioEnvelope = (serialized) => {
    if (typeof serialized !== 'string' || serialized.trim() === '')
        return null;
    try {
        const parsed = JSON.parse(serialized);
...
export const createRunId = (suiteId, planIndex, phase, recordedIndex = 0, attemptNumber = 1) => {
    const planLabel = `plan-${planIndex + 1}`;
    if (phase === 'warmup')
        return `suite-${suiteId}-${planLabel}-warmup`;
    return `suite-${suiteId}-${planLabel}-recorded-${recordedIndex + 1}-attempt-${attemptNumber}`;
};
export const getNextExecution = (session, plans) => {
    if (session.status !== 'running' || session.planIndex >= plans.length)
        return null;
    ...
    return {
        ...
        runId: createRunId(session.suiteId, session.planIndex, session.phase, session.recordedIndex, session.attemptNumber),
    };
};
export const prepareNextExecution = (session, plans) => {
...
const advanceRecordedCursor = (session, plans) => {
...
const incrementCompletedSlot = (session) => (Math.min(PERF_MAX_COMPLETED_SLOTS, session.completedSlots + 1));
const updateZeroRafStreak = (session, run) => {
...
const applyScenarioResult = (inputSession, pending, run, plans) => {
    ...
    const planMatches = run.scenario.viewport === plan.viewport
        ...
        && appendMatches;
    if (!planMatches)
        contextIssues.push('Scenario result does not match the active plan.');
    ...
    if (!timingValid) {
        ...
        const retryBlockedByContext = contextIssues.some(issue => (!issue.startsWith('Performance scenario must execute while document.visibilityState=visible.')));
        ...
        return {
            ...advanceRecordedCursor({
                ...
                failures: exhausted || !retryAllowed
                    ? [
                        ...session.failures,
                        scenarioFailure(plan, 'recorded', `${failureText(run, 'no timing-valid result was recorded')} (after ${inputSession.attemptNumber} attempt${inputSession.attemptNumber === 1 ? '' : 's'})`),
                    ]
                    : session.failures,
            }, plans),
            rejectedTimingAttempts: rejected,
...
const applyScenarioError = (inputSession, pending, plans) => {
    const plan = plans[inputSession.planIndex];
    if (!plan)
        return inputSession;
    const session = {
...
export const applyPendingExecution = (session, pending, plans) => {
    ...
    const nextSession = pending.type === 'result'
        ? applyScenarioResult(session, pending, pending.result, plans)
...
export const markViewportWaiting = (session, requirement) => ({
    ...session,
    status: 'waiting-for-viewport',
    fatalCode: null,
    fatalError: null,
});
export const completeSuite = (session) => ({
    ...session,
    status: 'completed',
    fatalCode: null,
    fatalError: null,
});
export const failSuite = (session, fatalCode, fatalError) => ({
    ...session,
    status: 'failed',
    fatalCode,
    fatalError,
});
export const buildPerfSuiteResult = (session, summary, decision) => ({
    status: 'completed',
