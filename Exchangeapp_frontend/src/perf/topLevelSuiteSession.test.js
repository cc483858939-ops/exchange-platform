...
import { applyPendingExecution, completeSuite, createRunId, createSuiteSession, executionContextValidationIssues, getNextExecution, isPerfPendingScenarioEnvelope, isPerfTopLevelSuiteSession, isViewportWithinTolerance, PERF_MAX_COMPLETED_SLOTS, PERF_ZERO_RAF_FATAL_CODE, parsePendingScenarioEnvelope, parseSuiteSession, prepareNextExecution, serializePendingScenarioEnvelope, serializeSuiteSession, } from './topLevelSuiteSession';
...
const pendingFor = (session, result, type = 'result') => ({
    schemaVersion: 2,
...
const apply = (current, result, type = 'result') => {
...
describe('top-level suite session state machine', () => {
    ...
    it('validates the shared viewport helper and environment consistency', () => {
        ...
        expect(executionContextValidationIssues(rawRun(plans[0], { userAgent: 'changed-agent' }), environment)).toContain('userAgent changed during the performance suite');
        expect(executionContextValidationIssues(rawRun(plans[0], { devicePixelRatio: 2 }), environment)).toContain('devicePixelRatio changed during the performance suite');
    });
