...
export function summarizeRafCadence(intervals) {
...
export function assessRafCadence(intervals, phase) {
...
export function assessTimingHealth(preflight, postScroll, postCleanup, scrollIssues, visibilityLost) {
    const postflightAssessment = assessRafCadence([...postScroll.intervals, ...postCleanup.intervals], 'postflight');
    const issues = [
        ...preflight.issues,
        ...postScroll.issues,
        ...postCleanup.issues,
...
const emptyLongTaskPhase = () => ({
    count: 0,
    longestMs: 0,
    totalMs: 0,
});
export function attributeLongTasksByPhase(entries, windows) {
    const summarizePhase = (phase) => {
        const window = windows[phase];
        if (!window) {
            return emptyLongTaskPhase();
        }
        const phaseEntries = entries.filter(entry => (Number.isFinite(entry.startTime)
...
export function assessScrollTiming(deltas, longTasks) {
...
const rowMatches = (row, other) => row.viewport === other.viewport
    && row.count === other.count
    && row.fixture === other.fixture
    && row.runType === other.runType;
const relativePercent = (value, baseline) => {
...
export function aggregatePerfRuns(rawRuns) {
...
const findMatrixRow = (summary, viewport, count, trackView, fixture = 'mixed') => summary.rows.find(row => row.runType === 'matrix'
    && row.viewport === viewport
    && row.count === count
    && row.trackView === trackView
    && row.fixture === fixture);
const REQUIRED_MATRIX_COUNTS = [20, 50, 100, 200, 300];
const hasCompleteRequiredCoverage = (rawRuns) => {
    const complete = (runs, requireAppend) => (runs.length >= 3
        && runs.every(run => (run.validation.valid
    ...
    for (const viewport of ['desktop', 'mobile']) {
        for (const trackView of [true, false]) {
            for (const count of REQUIRED_MATRIX_COUNTS) {
                const matrixRuns = rawRuns.filter(run => (run.scenario.viewport === viewport
            ...
            const appendRuns = rawRuns.filter(run => (run.scenario.viewport === viewport
...
const hasRatioAbove = (ratios, field, threshold) => (ratios.some(ratio => {
    const value = ratio[field];
    return typeof value === 'number' && value > threshold;
}));
const materiallyWorse = (tracked, untracked) => {
...
const materiallyDifferent = (left, right) => {
...
const rowMateriallyWorse = (tracked, untracked) => (materiallyWorse(tracked.medianMountMs, untracked.medianMountMs)
...
const isObservationBottleneck = (summary) => {
    ...
    const scalingDiverges = trackedScaling && untrackedScaling && (materiallyDifferent(trackedScaling.mountMs300To100, untrackedScaling.mountMs300To100)
...
const comparable = (left, right, tolerance = 0.2) => {
    if (left === null || right === null || !Number.isFinite(left) || !Number.isFinite(right)) {
        return false;
    }
...
const isMountedStateBottleneck = (summary) => {
    const tracked300 = findMatrixRow(summary, 'mobile', 300, true);
    const untracked300 = findMatrixRow(summary, 'mobile', 300, false);
...
const determineBottleneck = (summary, classification) => {
...
export function classifyPerfBaseline(rawRuns, summary) {
    ...
    const selectedRuns = rawRuns.filter(run => run.scenario.viewport === 'mobile'
        ...
        && run.scenario.runType === 'matrix');
    const recurringLongTaskOver100 = selectedRuns.filter(run => (run.longTasks.supported && (run.longTasks.longestMs ?? 0) > 100)).length >= 2;
    const recurringLongTaskOver200 = selectedRuns.filter(run => (run.longTasks.supported && (run.longTasks.longestMs ?? 0) > 200)).length >= 2;
    const observerLeak = rawRuns.some(run => (run.observer.currentTargets !== 0
        || (run.scenario.trackView
    ...
    const red = mobileTracked300.medianP95FrameMs > 50
        ...
        || observerLeak;
    const classification = green ? 'GREEN' : red ? 'RED' : 'YELLOW';
...
const display = (value) => (typeof value === 'number' && Number.isFinite(value) ? String(round(value)) : 'not supported');
const displayLongTaskPhase = (phase) => (phase
    ? `${display(phase.count)} / ${display(phase.totalMs)} / ${display(phase.longestMs)}`
    : 'not available');
export function serializePerfMarkdown(result) {
    const lines = [
