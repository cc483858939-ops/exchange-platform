...
import { attributeLongTasksByPhase, assessRafCadence, assessScrollTiming, assessTimingHealth, aggregatePerfRuns, classifyPerfBaseline, percentile, PERF_RAF_HEALTH_SAMPLES, serializePerfMarkdown, summarizeFrameDeltas, } from './metrics';
const rawRun = (count, trackView, options = {}) => ({
    scenario: {
        viewport: 'mobile',
        width: 390,
        height: 844,
...
const completeRecordedRun = (count, trackView, runType = 'matrix', appendFrom) => {
...
const withViewport = (run, viewport) => ({
    ...run,
    scenario: {
        ...run.scenario,
        viewport,
        ...
        for (const count of [20, 50, 100, 200, 300]) {
            for (const trackView of [true, false]) {
                for (let runIndex = 0; runIndex < 3; runIndex += 1) {
                    runs.push(withViewport(completeRecordedRun(count, trackView, 'matrix', appendCounts.has(count) ? count : undefined), viewport));
                }
            }
        }
        for (const trackView of [true, false]) {
...
const rafProbe = (phase, intervals) => ({
    intervals,
    ...assessRafCadence(intervals, phase),
});
describe('performance metrics', () => {
    it('accepts healthy 60Hz and 30Hz cadence but rejects 1Hz cadence', () => {
        const sixtyHz = assessRafCadence(Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 16.67), 'preflight');
        const thirtyHz = assessRafCadence(Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 33.33), 'preflight');
        const oneHz = assessRafCadence(Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 1000), 'preflight');
    ...
    it('invalidates visibility loss and postflight throttling', () => {
        const healthy = rafProbe('preflight', Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 16.67));
        const healthyPost = rafProbe('postflight after scroll', Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 16.67));
        const healthyCleanup = rafProbe('postflight after PostCard cleanup', Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 16.67));
        ...
        const throttledPost = assessTimingHealth(healthy, rafProbe('postflight after scroll', Array.from({ length: PERF_RAF_HEALTH_SAMPLES }, () => 1000)), healthyCleanup, [], false);
    ...
    it('distinguishes throttled scroll gaps from a long-task-backed real gap', () => {
        const throttled = assessScrollTiming([1000, 1000, 1000, 1000], { supported: false });
        const realLongTask = assessScrollTiming([500, 500, 500, 500], { supported: true, count: 4, longestMs: 500, totalMs: 2000 });
    ...
    it('aggregates long-task phase attribution when raw runs provide it', () => {
        const phaseRun = (mountMs, scrollMs) => {
    ...
    it('does not call a small-count tracked delta an observation bottleneck', () => {
        ...
        expect(decision.bottleneck).toBe('No single subsystem dominates this baseline; inspect the recorded matrix before changing architecture.');
    });
    ...
    it('classifies only valid recorded data and serializes the report', () => {
        ...
        ], aggregatePerfRuns([])).classification).toBe('MEASUREMENT NOT VERIFIED');
    });
});
