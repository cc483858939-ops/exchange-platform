...
describe('UserProfileView cursor pagination', () => {
    ...
    it('drops stale initial and load-more responses after changing profile', async () => {
        ...
        let resolveMore;
        const loadMoreA = new Promise((resolve) => {
            resolveMore = resolve;
        });
        mocks.getUser.mockImplementation((id) => id === '7' ? profileA : Promise.resolve(profile(8)));
        mocks.getUserPosts.mockImplementation((id, options) => {
    ...
    it('ignores a stale delete response after switching profile', async () => {
        let resolveDelete;
        mocks.getUserPosts.mockImplementation((id) => id === '8'
            ? Promise.resolve({ items: [post(8, 8)], next_cursor: null })
            : Promise.resolve({ items: [post(1, 7)], next_cursor: 'cursor-1' }));
        mocks.deletePost.mockImplementation(() => new Promise((resolve) => {
