...
import { RecommendationTelemetryClient, calculateViewportVisibility, } from './recommendationTelemetry';
...
const eventsFromAxios = (call) => call[1].events;
const eventsFromFetch = (call) => JSON.parse(String(call[1].body ?? '{}')).events;
...
class FakeIntersectionObserver {
    ...
    emit(element, intersectionRatio = 1, isIntersecting = intersectionRatio > 0) {
        const entry = {
...
describe('RecommendationTelemetryClient Feed dwell', () => {
    ...
    const flushKeepaliveEvents = async () => {
        ...
        return calls.length === 0 ? [] : eventsFromFetch(calls.at(-1));
    };
    beforeEach(() => {
        clock = 0;
        rafID = 0;
    ...
    it('enqueues dwell before click and flushes the pair', async () => {
        ...
        const events = eventsFromAxios(mocks.post.mock.calls[0]);
    ...
    it('stop finalizes dwell before keepalive flush', async () => {
        ...
        const events = eventsFromFetch(mocks.fetch.mock.calls[0]);
        expect(events).toEqual([expect.objectContaining({
                event_type: 'feed_dwell',
    ...
    it('ordinary observeCard does not create Feed dwell', async () => {
        ...
        const calls = mocks.fetch.mock.calls;
        if (calls.length > 0) {
            const events = eventsFromFetch(calls[0]);
            expect(events.map(event => event.event_type)).toEqual(['click']);
        }
        else {
            expect(mocks.post).toHaveBeenCalled();
            expect(eventsFromAxios(mocks.post.mock.calls[0]).map(event => event.event_type)).toEqual(['click']);
        }
    });
