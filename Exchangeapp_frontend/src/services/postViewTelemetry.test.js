...
import { PostViewTelemetryClient, resetPostViewTelemetryForTests, } from './postViewTelemetry';
...
describe('PostViewTelemetryClient', () => {
    ...
    it('splits more than 50 current-user events without mixing owners', async () => {
        ...
        mocks.post.mockImplementation(async (_url, body) => successResponse(body.events));
        restored.start();
        await settle();
        expect(mocks.post).toHaveBeenCalledTimes(2);
    ...
    it('drops malformed persisted records and caps the restored queue at 200', async () => {
        ...
        expect(saved.filter((item) => item.owner_user_id === 7)).toHaveLength(200);
        restored.stop();
    });
    it('qualifies a visible feed card once and preserves source', async () => {
        ...
        class FakeIntersectionObserver {
            ...
            disconnect() { }
            emit(target, ratio) {
                this.callback([{ target, intersectionRatio: ratio }], this);
            }
        }
